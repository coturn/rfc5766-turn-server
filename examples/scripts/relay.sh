...
# Other options:
# set bandwidth limit on client session 30000 bytes per second (--max-bps)
# use fingerprints (-f)
...
PATH="bin:../bin:../../bin:${PATH}" turnserver -L 127.0.0.1 -L ::1 -E 127.0.0.1 -E ::1 --max-bps=30000 -f -m 3 --min-port=32355 --max-port=65535 --no-tls --no-dtls 	
